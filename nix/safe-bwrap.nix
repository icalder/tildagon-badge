{
  ...
  bubble-wrap-rust = exe: let
    ...
    closure = pkgs.writeClosure (
      pkgs.writeText "all-deps" (toString [
        unsafe
        cargo-any-rust
      ])
    );
        ...
        ${
          ...
          then
            lib.concatMapStringsSep "\n" (s: "  --ro-bind ${s}\n            ${s}") (
              lib.filter (
                s: lib.isString s && s != "" && s != pkgs.rustc.outPath && s != pkgs.rustc.unwrapped.outPath
  ...
  in
    pkgs.writeShellApplication {
      ...
      text = lib.replaceStrings [original.passthru.rustc.outPath] [pkgs.emptyDirectory.outPath] (
